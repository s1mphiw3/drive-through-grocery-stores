...
if(isset($_POST['update_order'])){
   ...
   $update_orders = $conn->prepare("UPDATE `orders` SET payment_status = ?,status = ? WHERE id = ?");
   $update_orders->execute([$update_payment,'read', $order_id]);
...
if($_POST['update_order_status']){
   ...
   $update_orders = $conn->prepare("UPDATE `orders` SET order_status = ?,status = ?  WHERE id = ?");
   $update_orders->execute([$update_order_status,'read',$order_id]);
...
<section class="placed-orders">
   ...
   <div class="box-container">
      ...
      <?php
         $select_orders = $conn->prepare("SELECT * FROM `orders` where vendor_id = ? and status =? order by unix_timestamp(placed_on) desc ");
         $select_orders->execute([$vendor_id,'unread' ]);
      ...
      <div class="box">
		...
		<?php
		 ...
		 // $order_products[] = '';
		   ...
		   $cart_query = $conn->prepare("SELECT * FROM `order_items` WHERE order_id = ? ");
		   $cart_query->execute([$fetch_orders['id']]);
           
		   
			  while($cart_item = $cart_query->fetch(PDO::FETCH_ASSOC)){
				 $prod = $conn->prepare("SELECT * FROM `products` WHERE id = ?");
				 $prod ->execute([$cart_item['product_id']]);
					 while($product = $prod->fetch(PDO::FETCH_ASSOC)){	 
						 echo "(".$product['name']." x ".$cart_item['quantity'].")";
					  }; 
			  };
		   ?></p></span> 
